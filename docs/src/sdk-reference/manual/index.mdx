---
title: "NotteClient"
description: "The starting point for all operations"
---
import Client from '/snippets/client/index.mdx';
import AgentMdNotice from '/partials/agent-md-notice.mdx';

<AgentMdNotice />

### Installation

you can install the notte sdk using pip:

```bash
pip install notte-sdk
```

### Overview

With the notte sdk, everything starts by creating your NotteClient:

<Client/>

From there, you're able to create browser sessions, run new agents, or manage credential vaults:

## Available Operations

<CardGroup cols={3}>
  <Visibility for="humans">
    <Card
      title="Sessions"
      icon="browser"
      href="/sdk-reference/manual/session"
    >
      List and manage browser sessions
    </Card>
  </Visibility>
  <Visibility for="agents">
    <Card
      title="Sessions"
      icon="browser"
      href="/sdk-reference/manual/session.md"
    >
      List and manage browser sessions
    </Card>
  </Visibility>
  <Visibility for="humans">
    <Card
      title="Agents"
      icon="robot"
      href="/sdk-reference/manual/agent"
    >
      List and manage automation agents
    </Card>
  </Visibility>
  <Visibility for="agents">
    <Card
      title="Agents"
      icon="robot"
      href="/sdk-reference/manual/agent.md"
    >
      List and manage automation agents
    </Card>
  </Visibility>
  <Visibility for="humans">
    <Card
      title="Functions"
      icon="book"
      href="/sdk-reference/manual/workflow"
    >
      Create and manage functions for your automations
    </Card>
  </Visibility>
  <Visibility for="agents">
    <Card
      title="Functions"
      icon="book"
      href="/sdk-reference/manual/workflow.md"
    >
      Create and manage functions for your automations
    </Card>
  </Visibility>
</CardGroup>


## Available Tooling for your agents

<CardGroup cols={3}>
  <Visibility for="humans">
    <Card
      title="File Storage"
      icon="file"
      href="/sdk-reference/manual/file_storage"
    >
      Upload and download files to and from your agents
    </Card>
  </Visibility>
  <Visibility for="agents">
    <Card
      title="File Storage"
      icon="file"
      href="/sdk-reference/manual/file_storage.md"
    >
      Upload and download files to and from your agents
    </Card>
  </Visibility>
  <Visibility for="humans">
    <Card
      title="Personas"
      icon="user"
      href="/sdk-reference/manual/persona"
    >
      Create and manage personas
    </Card>
  </Visibility>
  <Visibility for="agents">
    <Card
      title="Personas"
      icon="user"
      href="/sdk-reference/manual/persona.md"
    >
      Create and manage personas
    </Card>
  </Visibility>
  <Visibility for="humans">
    <Card
      title="Vaults"
      icon="lock"
      href="/sdk-reference/manual/vault"
    >
      Create and manage secure credential vaults for your agents
    </Card>
  </Visibility>
  <Visibility for="agents">
    <Card
      title="Vaults"
      icon="lock"
      href="/sdk-reference/manual/vault.md"
    >
      Create and manage secure credential vaults for your agents
    </Card>
  </Visibility>
</CardGroup>
